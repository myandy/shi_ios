...
public class EditPagerView : UIView,Nibloadable{
    ...
    public override func draw(_ rect: CGRect) {
        ...
        for i in 0...clist.count-1{
            let itemTop=70*i+10
            let frame = CGRect(x:10,y:itemTop,width:Int(self.bounds.width-10),height:20)
            ...
            let frame1 = CGRect(x:10,y:itemTop+30,width:Int(self.bounds.width-10),height:20)
            let textField=UITextField(frame:frame1)
            textField.textColor=UIColor.lightGray
            textField.tintColor=UIColor.lightGray
            srcollView.addSubview(textField)
            
            let lineView = UIView(frame:CGRect(x:10,y:itemTop+49,width:Int(self.bounds.width-10),height:1))
